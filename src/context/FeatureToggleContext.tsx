...
const FeatureToggleContext =
  createContext<FeatureToggleContextDefault>(ftDefault);
...
const FeatureToggleContextProvider = ({
  ...
  const isEnabled = (feature_name: Feature) => {
    return _.includes(features, feature_name.toString());
  };

  useEffect(() => {
    const str_raw_features = (process.env.NEXT_PUBLIC_ENABLED_FEATURES ??
      "") as string;
    setFeatures(_.split(str_raw_features, ",").map((f) => f.trim()));
  }, []);

  return (
    <FeatureToggleContext.Provider value={{ isEnabled }}>
      {children}
